...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    intervalHandle = setInterval(() => {
      ...
      try {
        ...
        // instead of throwing+logging on every tick (issue #95).
        if (isStaleCtxError(err)) {
  ...
  function worktreeSpawnFn(args: {
    ...
    try {
      ...
      // Defensive: a malformed `config.json` here cannot prevent the
      // status repaint (we're already in a setup-failed catch handler
      // and need to surface SOME signal to the user). Default
      // `showStatus` to `true` if `loadVaultConfig` itself errors, and
      // surface the underlying error via notify so the user knows the
      // file needs a hand-fix rather than seeing only the status
      // repaint.
      let showStatus = true;
      try {
  ...
  // /distill-status
  //
  // User-driven status surface. The command reads active worktrees +
  // unmerged branches via module-level `collectDistillStatus` and formats
  // them for humans via `formatDistillStatus`. The formatter is
  // module-level + exported so tests can verify its output without wiring
  // a full mock `ExtensionAPI`.
  //
  // Agent visibility for in-flight distills is handled proactively by the
  // per-distill-completion overlap notice posted via
  // `appendCustomMessageEntry` — there is no agent-callable tool because
  // worktree isolation means the agent can't race an in-flight distill on
  // main anyway, and the overlap notice fires exactly when files actually
  // change in the parent's view.
  // ---------------------------------------------------------------------------
...
/**
 * Gather the status payload consumed by `/distill-status`. Empty arrays on
 * any error (no git, git failure) — status surfaces must never throw.
 *
