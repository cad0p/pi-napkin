...
#   of sleep between attempts (~7.5s worst-case wall wait with command
#   runtime factored in). Good for transient contention; preserves
#   visible failures for real errors (nothing is retried when index.lock
#   isn't the problem, because git's own exit code is deterministic
#   across retries of the same state... in practice, retrying always
#   costs <1s total because contending commits are fast).
#
